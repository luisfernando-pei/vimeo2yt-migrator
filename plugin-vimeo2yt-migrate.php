...
function vimeo2yt_get_candidates(WP_REST_Request $req)
{
  ...
  foreach ($q->posts as $post) {
    ...
    $items[] = [
      ...
      'post_date' => (string) $post->post_date, // Data de criação do post (para ordenação)
    ];
  }
...
function vimeo2yt_update_youtube(WP_REST_Request $req)
{
  ...
  if (empty($meta_key)) {
    $meta_key = 'url_do_youtube';
  }

  // Verifica se a função do ACF existe, falhando de forma segura antes de prosseguir
  if (!function_exists('update_field')) {
    return new WP_REST_Response(['ok' => false, 'error' => 'acf_not_installed'], 500);
  }

  // Atualiza ACF diretamente
  update_field($meta_key, $youtube_url, $post_id);
